...
public class QorseRender extends GeoEntityRenderer<Qorse> {
    ...
    public QorseRender(EntityRendererProvider.Context renderManager) {
        super(renderManager, new QorseModel());
        this.addRenderLayer(new QorseMarkingLayer(this));
        this.addRenderLayer(new QorseStripesLayer(this));
        this.addRenderLayer(new QorseSaddleLayer(this));
    }

    @Override
    public void preRender(PoseStack poseStack, Qorse entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
