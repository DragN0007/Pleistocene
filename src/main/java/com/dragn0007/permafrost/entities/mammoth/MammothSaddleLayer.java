...
@OnlyIn(Dist.CLIENT)
public class MammothSaddleLayer extends GeoRenderLayer<Mammoth> {
    public MammothSaddleLayer(GeoRenderer<Mammoth> entityRendererIn) {
    ...
    public void render(PoseStack poseStack, Mammoth animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if (animatable.isSaddled()) {
            resourceLocation = new ResourceLocation(Permafrost.MODID, "textures/entity/mammoth/tack/mammoth_saddle.png");
        } else {
            return;
        }
